...
/**
 * User-task handler for entering parameters in the dmn-credit flow.
 */
@Service
@Slf4j
public class UserTaskServiceEnterDmnCreditParameters extends GenericUserTaskService {
	 ...
	 */
	public UserTaskServiceEnterDmnCreditParameters(TaskService taskService, RuntimeService runtimeService, HistoryService historyService) {
		super(taskService, runtimeService, historyService);
	}

	/**
	 * Completes the dmn-credit parameter task.
	 *
	...
	public void enterCreditParameters(BigDecimal monthlyNetIncome, BigDecimal propertyValue, BigDecimal equity, String processInstanceId) {
		...
		Optional<Task> taskOpt = super.findTask(processInstanceId, "Task_EnterDmnCreditParameters");
		if (taskOpt.isEmpty()) {
			throw new RuntimeException("[%s] No active Task_EnterDmnCreditParameters was found.".formatted(processInstanceId));
